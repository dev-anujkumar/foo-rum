...
const Tooltip: React.FC<TooltipProps> = ({
  ...
  return (
    <div className={`relative group ${className}`}>
      {children}
      <div 
        className={`absolute ${getPositionClasses()} px-2 py-1 bg-gray-900 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-${delay} pointer-events-none whitespace-nowrap z-10`}
      >
        {content}
        {showArrow && <div className={getArrowClasses()}></div>}
      </div>
    </div>
  );
};
